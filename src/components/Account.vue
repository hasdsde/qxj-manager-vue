<template>
    ...
    <div class="q-mr-sm">{{ '用户： ' + userInfo.name }}</div>
        ...
        <q-card style="min-width: 20%;">
            ...
            <q-card-section>
                <q-form class="q-gutter-md">
                    <q-input class="q-pa-sm" filled v-model="old_pwd" :rules="[val => val && val.length > 0 || '必填']"
                             label="旧密码"/>
                    <q-input class="q-pa-sm" filled v-model="new_pwd"
                             :rules="[value => value && value.length > 0 || '必填']"
                             label="新密码"/>
                    <q-input class="q-pa-sm" filled v-model="confirm_pwd"
                             :rules="[val => val && val.length > 0 || '必填', value => value === new_pwd || '旧密码与新密码不一致']"
                             label="确认密码"/>
                </q-form>
            </q-card-section>
...
import {CommonFail, CommonWarn, Success} from 'src/components/commonResults'
...
function getUserInfo() {
    if (localStorage.getItem("userInfo") == null) {
        ...
        //@ts-ignore
        userInfo.value = JSON.parse(localStorage.getItem("userInfo").toString())
    }
}
